  ...
  return (
    <header className="bg-white border-b border-slate-200">
      <div className="w-full px-4 md:px-8 h-14 flex items-center justify-between">
        ...
        {title && <span className="text-sm font-medium text-slate-700 hidden md:block">{title}</span>}
        <div className="flex items-center gap-3">
          {user && <span className="text-sm text-slate-500 hidden sm:block">{user.name}</span>}
          <button onClick={logout} className="flex items-center gap-1.5 text-sm text-slate-600 hover:text-slate-900 px-3 py-1.5 rounded-lg hover:bg-slate-100 transition-colors">
            <LogOut size={15} />
            Logout
          </button>
        </div>
