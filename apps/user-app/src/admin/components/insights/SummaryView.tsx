        ...
        <Tooltip key={bucket.value} title={`${bucket.value}: ${bucket.count}`}>
          <div className="flex flex-col items-center justify-end w-4 h-10">
            <div
              ...
              style={{ height: `${Math.max((bucket.count / maxCount) * 100, bucket.count === 0 ? 6 : 10)}%` }}
            />
                    ...
                    r.scopeType === "option" ? (
                      ...
                      <div>
                        <span className="text-sm text-slate-600">
                          avg {r.numeric?.avg.toFixed(1)} · min {r.numeric?.min} · max {r.numeric?.max} · n={r.numeric?.count}
                        </span>
                        {r.numeric?.distribution && <NumericDistribution distribution={r.numeric.distribution} />}
                      </div>
                    ),
                },
              ]}
